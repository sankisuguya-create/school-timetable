...
const Gate = (function(){
  ...
  /* **設定は、教職員なら誰でも触れる。**
     前はここで「管理者だけ」を足していたが、学校で3人しか直せないと、
     基本時間割や学級編成を直したい人が、その3人の手が空くのを待つことになった。
     いま止めるのは**児童と外の人**だけ（check の1段目）。

     checkAdmin そのものは残してある ── 呼ぶ側はもう無いが、
     「管理者」という考え方は `管理・システム` の画面と apiBoot の
     isAdmin で生きている（どの版がどのファイルにつながっているかを見る面）。
     もう一度どこかを管理者だけに戻したくなったら、ここを呼ぶ。 */
  function checkAdmin(){
    const j = check();
    if(!isAdmin(j.email))
