...
const Store = (function(){
  ...
  function tpOpen(url){
    ...
    /* 呼び出し側が渡したURLをそのまま開かない。ウェブアプリは設置者権限で動くため、
       任意URLを許すと、一般教員が設置者のアクセスできる別ファイルを書き換えられる。 */
    const allowed = targets.some(function(x){
      try{ return fileId(x.url, "URL", "たんぽぽの出す先の") === id; }
      catch(e){ return false; }
    });
    if(!allowed) throw new Error("登録されていない出す先には書き込めません。出す先を読み直してください");
    let ss;
    try{ ss = SpreadsheetApp.openById(id); }
    catch(e){
  ...
  function exportPlanSheet(name, sheets){
    ...
    sheets.forEach((part, i) => {
      ...
      if(values.length){
        ...
        /* 週案の字を数式として評価しない。先に text 書式へしてから値を置く。 */
...
function apiBoot(year){
  const me = Gate.check();
  const allConfig = Store.readConfig();
  const publicConfig = {};
  ["印刷用紙", "印刷余白mm", "印刷倍率", "タイトル文字pt", "詳細文字pt",
   "時数_貼る先", "時数_クラスの順", "時数_1日の行数", "時数_列のずれ",
   "A週の起点の月曜"].forEach(function(k){ if(k in allConfig) publicConfig[k] = allConfig[k]; });
  const out = {
    me:       me.email,
