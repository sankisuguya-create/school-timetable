...
let res = ev(`Store.writeCells(2026, [
  ...
  {date:"2026-11-16", slot:"p2", layer:"home",   target:"3-3", title:"算数", note:"わり算", subject:"sansu", short:"わ"},
...
/* 時数名。**手で決めた1文字の控え。** 「専科」の教科コード分割と同じく、
   古いファイルには無い列なので、無い前提のシート（このすぐ下の節）も見る */
ok("時数名が戻る", w.home["3-3"]["0|p2"].short === "わ", w.home["3-3"]["0|p2"]);
ok("書いていないコマは空", w.home["3-3"]["4|p1"].short === "", w.home["3-3"]["4|p1"]);

console.log("\n■ 時数名の列が無い、前の版のシートでも壊れない");
/* **列を後ろへ足しただけの「専科」と同じ考え方。** 見出しが13列そろっていない
   既存シートに書くとき、ensurePlan が右へ見出しを足してから書く。
   足さずに書くと、値は入っても読むときに見出しが無いので永遠に読めない
   （→ gas/Sheets.gs ensurePlan）。ここでは見出しを12列（時数名の手前まで）
   だけにしたシートを、はじめから作っておいて確かめる。 */
SHEETS["週案 4-1"] = [["年度","日付","曜日","時程","題名","詳細",
  "教科コード","層","対象","担当","更新者","更新時刻"]];
ev(`Store.writeCells(2026, [
  {date:"2026-11-16", slot:"p1", layer:"home", target:"4-1",
   title:"社会", subject:"shakai", short:"社"}
])`);
ok("見出しが13列に足りる（時数名が足される）",
   rowsOf("週案 4-1")[0][12] === "時数名", rowsOf("週案 4-1")[0]);
ok("値も正しい列に入る（見出しの右に書いただけで、既存の列はずれない）",
   rowsOf("週案 4-1")[0][4] === "題名" && rowsOf("週案 4-1")[1][4] === "社会",
   rowsOf("週案 4-1"));
const w41 = ev('Store.readWeek(2026, "2026-11-16")');
ok("読み直しても時数名が届く",
   w41.home["4-1"]["0|p1"].short === "社", w41.home["4-1"]);
