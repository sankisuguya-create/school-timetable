...
const Backend = (function(){
  ...
  /* baseAt ＝ **直す前に、そのコマがサーバで持っていた時刻。**
     呼ぶ側（writecell.js の writeCell）が、書き替える前に読んで渡す。
  ...
  /* **前後の週も待たせない。** 週案は翌週を書くことがいちばん多いので、
     「次の週」を押すたびに1往復待っていた。いま見ている面のぶんだけ、
     前後の週を apiReadWeeks の1往復で読んでおく（シートは1回しか走査しない
     ので、サーバの手間は1週ぶんとほぼ同じ）。**同じ年度の週だけ**
     （readWeeks は年度ごとに読む）。一度も読んでいない週だけ読む。 */
  let nbT = 0;
  function prefetchNeighbors(){
    if(!onGas) return;
    clearTimeout(nbT);
    nbT = setTimeout(() => { if(!sending) prefetchNeighbors_(fy(), editEpoch); }, 1200);
  }
  function prefetchNeighbors_(year, epoch){
    const vw = targetsForView();
    if(!vw.length) return;
