...
const LAYER_NAME = {base:"", tent:"仮", school:"全体", grade:"学年", special:"専", home:""};
const LAYER_FULL = {base:"基本時間割", tent:"仮採用", school:"学校全体", grade:"学年",
                    special:"専科", home:"担任"};
/* **仮採用は、基本時間割のすぐ上。** 誰かが本物のコマを書いた瞬間に負ける
   （→ TENT_SLOT）。順そのものは同じ時刻に並んだときにしか使わない。 */
const RANK = {base:0, tent:0.5, home:1, special:2, grade:3, school:4};

/* ── 仮採用（専科の月予定） ────────────────────
   **基本時間割と同じ薄さ・同じ低さで入れる。** 案を「だいたいこれで行く」と
   置いておきたいが、誰かが本物の予定を入れたらそちらが勝つ、という置き方。

   持ち方は**ふつうのコマと同じ**（時程のIDに `tent:時程`）。`day`・`memo`・
   `trip:` と同じ手で、専用の入れ物も週案シートの列も作らない。だから
   書き込み・読み込み・競合・まだ送っていない数え方・年度の退避が全部効く。

   **本物の校時とは別の行に入る**ので、誰かがその校時へ書いても競合しない。
   合成（compose）は本物の校時を先に見て、そこが基本時間割のままのときだけ
   仮採用を出す ── これで「優先度がいちばん低い」が構造として決まる。
   紙に出ている以上、時数もたんぽぽも基本時間割と同じように数える。 */
const TENT_SLOT = "tent:";
