...
async function nodePng(node, name){
  ...
  if(r.width < 1 || r.height < 1)
    throw new Error("いま見えていない面は画像にできない。その面を開いてから押す");
...
const HELP = {
  ...
  spmonth: {t:"専科の月予定をくむ（仮）",
    b:["<b>1ヶ月ぶんの専科の巡りを、干渉が起きないように計算して出します。</b>"
       ...
       + "破るのは「そうしないと、そのコマをどこにも置けない」ときだけです。"
       ...
       + "業間が2と3のあいだ、給食と昼休みが4と5のあいだに入るので、"
       + "23 と 45 は続きになりません。",
       ...
       "<b>希望は月ごとに持ちます。</b>空のときは先月のぶんを既定にするので、"
  ...
  pall: {t:"5日とも同じにする",
    b:["<b>朝学習・業間・昼休みの行にだけ出ます。</b>"
       ...
       + "5日を同じにする場面がありません。",
       "<b>いま選んでいる欄の題名・備考を、その週の5日ぶんへ一気に写します。</b>"
       + "「朝学習」の中身を月曜だけ書いて、あとの4日はコピーする手間を無くします。",
