...
const HELP = {
  week: {t:"週を行き来する・A週B週",
    b:["<b>今見ている1週間です。</b>◀ ▶ で前・次の週へ。カレンダーの印では、日を選んでその週へ飛びます。",
       "<b>A週・B週</b> は日付から自動で決まるので、ふだんは押しません。",
       "一番下の欄で、見るクラス・学年・専科を選びます。"]},
  gate: {t:"メニューへ",
    b:["<b>クラスや学年を選ぶ、入口の表です。</b>押すとその週案が開きます。",
       "<b>開いた面が、そのまま予定の入る先です。</b>"
       + "「3-3」で書けば3年3組だけ、「3年」なら3年の全クラス、「全学年」なら全クラスに入ります。"]},
  now: {t:"今：◯◯",
    b:["<b>今開いているものの名前です。</b>押すとそこへ戻ります。"]},
  print: {t:"印刷（B5）",
    b:["<b>今の1週間を B5 の紙1枚に刷ります。</b>",
       "刷られるのは白い紙の部分だけで、まわりのボタン類は入りません。"]},
  tally2: {t:"時数を集計する",
    b:["<b>今開いているクラスの授業を教科ごとに数え、「時数集計」シートに置きます。</b>",
       "左が今月、右が年度始め4/1からの累計です。",
       "まだ書いていない日は基本時間割どおりとして数えます。",
       "学年の面から入れるとその学年の全クラスを数えます。"
       + "年度の後半は読む週が増えるので、月末・学期末に1回押す使い方です。"
       + "置いたものは、押すたびにそのクラスの行だけ作り直します。"]},
  tally: {t:"時数をコピー",
    b:["<b>今の1週間の授業を、時数集計表に貼れる形でコピーします。</b>"
       + "Excel の表を開いて貼り付けてください。"]},
  newyear: {t:"新年度設定",
    b:["<b>年度の初めにやることを順に並べた画面です。</b>上から1つずつやれば終わります。",
       "元に戻せない操作は ⚠ が付いた1つだけで、押す前に必ず確認が入ります。"]},
  tanpopo: {t:"たんぽぽ",
    b:["<b>たんぽぽ学級へ渡す時間割を1週ぶん出す画面です。</b>",
       "組の並びや人数は右上の「組分けを直す」で変えます（年度の初めと転入出のときだけ）。",
       "クラスの「済／未」は、その担任が「たんぽぽに提出」を押したかどうかです。",
       "書き入れると「◯月◯週」のシートができます。同じ名前があれば名前を変えて残します。"]},
  specials: {t:"専科の枠",
    b:["<b>1行が1人ぶんです。</b>教科と担当学年を決めます。",
       "同じ教科を学年で分けても構いません（理科3・4年と理科5・6年 など）。",
       "担当学年に合うコマだけがその人の週に出ます。空欄なら全学年です。",
       "書き方は 3,4 でも 3〜6 でも通ります。"]},
  free: {t:"空き枠あり/なし表示",
    b:["<b>どのコマが空いているかを、紙の上に印で出します。</b>学年・全学年・専科の面にだけ出ます。",
       "全校・学年枠＝動かせない予定（全校・学年の予定・校外・授業なし）に印を付けます。",
       "全校・学年・他専科枠＝それに、他の専科と避ける教科を足します。",
       "色ではなく斜線と記号（△ ×）で分けます。使わないときは畳んで置けます。"]},
  spmonth: {t:"専科の月予定を組む",
    b:["<b>1ヶ月ぶんの専科の巡りを、ぶつからないように計算して出します。</b>"
       + "「組む」を押すまで、そして「入れる」を押すまで、週案には何も書きません。",
       "各専科の持ちコマ数は基本時間割から数えます。入れてもらう数字はありません。",
  ...
  dayform: {t:"この週の日の形",
    b:["<b>その日を特別校時・休みにします。全学年の面からだけ直せます。</b>",
       "特別校時＝朝学習の行が消えて、下が上へ詰まります。",
       "休み＝1〜6校時に斜め線。中身は消えず、戻せばそのまま出ます。"]},
  gradeview: {t:"学年でならべる",
    b:["<b>同じ学年のクラスを横に並べて見る画面です。</b>"
       + "合同体育や専科の巡りがそろっているか分かります。",
       "コマの中身は教科の1文字です。ここは見るだけで、直すのは週の紙で。"]},
  cal: {t:"カレンダー（2ヶ月・A4横）",
    b:["<b>2ヶ月ぶんを A4 横1枚に並べて見る画面です。</b>そのまま刷れます。",
       "1日は、日付の下に校時が上から順に並びます。左が教科の1文字、右が書き込み欄"
       + "（専科の面では左に行き先のクラス名が出ます）。",
       "上位から降りたコマは左端に線が付きます（全校＝青・学年＝橙・専科＝茶）。",
       "「備考：空欄／出す」で右の欄を切り替えます。月の下の数字はその月のコマ数です。"]},
  renraku: {t:"連絡帳用（A4横）",
    b:["<b>児童の連絡帳・掲示用の1枚です。</b>日付を選ぶとその日のぶんを縦書き・ルビつきで出します。",
       "右から 日付曜日・つぎの日の時間割・宿題・持ち物。宿題と持ち物は窓の中で書きます。",
  ...
  trip: {t:"この校外行事の名前",
    b:["<b>紙に出す字を、この行事だけに決めます。</b>",
       "続けて置いた1本ぶんに同じ字が入ります。紙では12文字まで、たんぽぽでは2文字までです。"]},
  events: {t:"この日の行事",
    b:["<b>年間行事計画表から読んだ、その日の行事です。</b>",
       "コマを選んでから押すと入ります。何校時に入れるかは、押す人が決めます。"]},
  links: {t:"リンク",
    b:["<b>コマの文字に開く先を付けられます。</b>文字を選んでから下のボタンを押します。",
       "画面ではその文字を押すと開きます。「外す」で消せます。"]},
  pall: {t:"5日とも同じにする",
    b:["<b>朝学習・業間・昼休みの行にだけ出ます。</b>",
       "今の欄の題名・備考を、その週の5日ぶんへ写します。上書きになる日は先に聞きます。",
       "休み・ロック中の日には書きません。"]},
  fontsize: {t:"フォントサイズ",
    b:["<b>紙に出る字の大きさです。</b>題名と備考を別々に変えられます。",
       "設定の「印刷と文字」と同じものです。"]},
  tally3: {t:"時数",
    b:["<b>今の面の授業を教科ごとに数えたものです。</b>左が今月、右が年度からの累計。",
       "書いていない日は基本時間割どおりとして数えます。",
  ...
  impev: {t:"年間行事からコマを作る",
    b:["<b>行事計画を、全校・学年のコマにします。</b>",
       "「連携シートを開く」で専用シートを開き、行事計画をそのまま貼ります。",
       "「連携シートから読む」で読み取り、入れない行のチェックを外して「入れる」を押します。",
       "校時・対象は字から読みます（4h→4校時、AM→1〜4校時、12時台下校→5・6校時授業なし、"
       + "2-6年→2〜6年）。読めない行は飛ばします。"]},
  month: {t:"月で見る（B4）",
    b:["<b>今の週から4週ぶんを2×2で見る画面です。</b>B4横1枚に刷れます。",
       "ここは見るだけです。直すのは週の紙で。"]},
  base: {t:"基本時間割",
    b:["<b>毎週くり返す、もとの時間割です。</b>",
       "直すと、まだ書いていない週の見え方が変わります（書いた予定は変わりません）。",
       "A週・B週の2種類を持てます。表ごと貼って取り込めます。"]},
  settings: {t:"設定",
    b:["<b>学校全体と年度のもとになる設定の画面です。</b>"]},
  roster: {t:"学級編成",
    b:["<b>学年とクラス・専科・たんぽぽの交流級を決めます。</b>"
       + "ここで決めたものが入口の表になります。"]},
  paper: {t:"用紙・印刷",
    b:["<b>刷る紙のサイズ・余白・倍率を決めます。</b>画面の大きさもここで変えられます。"]},
  legend: {t:"紙の上の見え方",
  ...
  admin: {t:"管理・システム",
    b:["<b>担任の操作はここにありません。</b>版とファイルのつながりを見るところです。",
       "不具合の連絡には、この窓の下の1行をそのまま伝えてください。",
