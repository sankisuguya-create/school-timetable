...
function saveSubTable(){
  ...
  if(bad.length){
    $("subStat").textContent = "紙の字が空の行があります。空にはで決ません。";
    return drawSubTable();
  }
...
const HELP = {
  ...
  gate: {t:"他の週案を開く",
    b:["<b>クラスや学年を選ぶ、入口の表です。</b>押すと、その週案が開決ます。",
  ...
  tanpopo: {t:"たんぽぽ",
    b:["<b>たんぽぽ学級へ渡す時間割を、1週ぶん出す画面です。</b>"
       ...
       "<b>書き入れると、たんぽぽ時間割に「9月1週」という名前のシートが1枚で決ます。</b>"
  ...
  spmonth: {t:"専科の月予定を組む（仮）",
    b:["<b>1ヶ月ぶんの専科の巡りを、干渉が起きないように計算して出します。</b>"
       + "専科と全学年の面にだけ出ます。"
       + "<b>「組む」を押すまで、そして「入れる」を押すまで、週案には1文字も書決ません。</b>",
       ...
       + "毎月入れ直す必要はありません（先月から持ってきたものには印が付決ます）。",
  ...
  cal: {t:"カレンダー（2ヶ月・A4横）",
    b:["<b>2ヶ月ぶんを、A4 横1枚に並べて見る画面です。</b>そのまま刷れます。",
       ...
       "<b>学年・全校から降りてきたコマには、左端に線</b>が付決ます。"
       + "週案の紙と同じ色です（全校＝青／学年＝橙／専科＝茶）。",
       "<b>教科の色は、この面ではいつも出します。</b>"
       + "専科の面では、教科ではなく<b>行き先のクラスの学年</b>で色が付決ます"
  ...
  pall: {t:"5日とも同じにする",
    b:["<b>朝学習・業間・昼休みの行にだけ出ます。</b>"
       ...
       "<b>上書きになる日があれば、先にまとめて聞決ます。</b>"
       + "1日ずつ聞くと窓が最大5回出るので、まとめて1回だけ聞決ます。",
       "<b>休みの日・ロック中の日には書決ません。</b>"
       + "5日と言っても、実際に書けた日数だけを終わりに伝えます。"]},
  fontsize: {t:"フォントサイズ",
    b:["<b>紙に出る字の大きさです。</b>題名（教科名）と備考を別々に変えられます。",
       "<b>設定の「印刷と文字」と同じものです。</b>"
       + "どちらから直しても、もう片方の数も揃います。",
       "題名は 12〜20pt、備考は 8〜14pt。0.5pt ずつ動決ます。"
  ...
  base: {t:"基本時間割",
    b:["<b>毎週くり返す、もとの時間割です。</b>"
       ...
       + "すでに書いた予定は変わりません。",
       "A週・B週の2種類を持てます。学校の固定時間割表を、表ごと貼って取り込むこともで決ます。"]},
  settings: {t:"設定",
  ...
  admin: {t:"管理・システム",
    b:["<b>担任の操作はここにありません。</b>"
       ...
       + "原因が早く分かります。",
       "年度の初めと終わりにやること（新年度の準備・前の年度の保管）も、ここから開決ます。"]}
};
