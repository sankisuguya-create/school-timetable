...
/* 字の列（segs）を縦に1列書く。**ルビは字の右の細い筋へ。**
   返すのは使った高さ。x は字の中心。 */
function renVText(ctx, x, y, segs, fs, color){
  ...
  for(const sg of segs){
    ...
    for(let i = 0; i < t.length; i++){
      const ch = t[i];
      if(REN_DIGIT.test(ch)){                    /* 数字・ローマ字は縦中横 */
        let run = ch;
...
/* 日付の読み（カッコ書きで書かせないよう、こちらで正しい読みを持つ） */
const REN_MON = ["","いちがつ","にがつ","さんがつ","しがつ","ごがつ","ろくがつ",
                 "しちがつ","はちがつ","くがつ","じゅうがつ","じゅういちがつ","じゅうにがつ"];
const REN_DAY = ["","ついたち","ふつか","みっか","よっか","いつか","むいか","なのか",
...
/* 教科・よく出る行事の読み。**ここに無い字にはルビを出さない**
   （当てずっぽうの読みを刷るより、ルビなしのほうがまだ書き写せる）。 */
const REN_YOMI = {
  ...
  try{
    ...
    if(/休/.test(dayT)) letters.push({t:"休み", rt:REN_YOMI["休み"]});
      ...
      for(const s of SLOTS){
        ...
        const sub = c.subject && SUB_BY_CODE[c.subject];
        letters.push({t:lt, rt:sub ? (REN_YOMI[sub.name] || "") : (REN_YOMI[t] || "")});
      }
    }
    const lines = s => String(s || "").split("\n").map(x => x.trim()).filter(x => x).map(renRuby);
    const dowCh = dt.getDay() === 0 ? "日" : DOW[d];
    return {dt, letters,
      date:[{t:(dt.getMonth()+1) + "月", rt:REN_MON[dt.getMonth()+1]},
            {t:dt.getDate() + "日", rt:REN_DAY[dt.getDate()]},
...
/* 見出し＋中身の列を「箱」1つに収めて書く。右から左へ積む。
   返すのは箱が食った横幅（次の箱の右端を決める）。 */
function renBox(ctx, xr, y, head, cols, headFs, bodyFs, color, minH){
  const pad = 18, gap = 8;
  const wOf = (segs, fs) => fs + (segs.some(s => s.rt) ? fs * 0.62 : 0);
  /* 見出しの筋は箱の右はし。ルビはさらにその右 */
  let x = xr - pad - wOf(head, headFs);
  const hx = x + headFs / 2;
  const hHead = renVText(ctx, hx, y + pad, head, headFs, color);
  let hMax = Math.max(hHead, minH || 0);
  x -= gap;
  for(const col of cols){
    x -= wOf(col, bodyFs);
    if(col.length){
      const h = renVText(ctx, x + bodyFs / 2, y + pad, col, bodyFs, color);
      hMax = Math.max(hMax, h);
    }
    x -= gap;
  }
  const left = x - pad + gap, bottom = y + pad + hMax + pad;
  ctx.save(); ctx.strokeStyle = "#7c93b8"; ctx.lineWidth = 3;
  if(ctx.roundRect){ ctx.beginPath(); ctx.roundRect(left, y, xr - left, bottom - y, 14); ctx.stroke(); }
  else ctx.strokeRect(left, y, xr - left, bottom - y);
  ctx.restore();
  return (xr - left) + 34;      /* 箱と箱のあいだ */
}
...
function renDraw(m){
  ...
  /* 上端の青い罫線（手元の画像の形。日付・時間割を書き写す帯） */
  ctx.strokeStyle = "#8fb0f0"; ctx.lineWidth = 5; ctx.lineCap = "round";
  ctx.beginPath(); ctx.moveTo(70, 118); ctx.lineTo(1270, 118); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(70, 168); ctx.lineTo(1270, 168); ctx.stroke();

  let xr = W - 52, y = 210;
  /* いちばん右は日付曜日（大きく・箱なし） */
  renVText(ctx, xr - renColW(m.date, 52) + 26, y, m.date, 52, "#222");
  xr -= renColW(m.date, 52) + 56;

  xr -= renBox(ctx, xr, y, m.headTT, [m.letters], 32, 46, "#222");
  xr -= renBox(ctx, xr, y, m.headHW, m.hw.length ? m.hw : [[]], 32, 34, "#222", 380);
  xr -= renBox(ctx, xr, y, m.headIT, m.items.length ? m.items : [[]], 32, 34, "#222", 380);
