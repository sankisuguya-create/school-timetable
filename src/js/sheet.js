...
/* 携帯では、紙を枠のかたちに合わせて潰さない。**刷ったときと同じ縦横**の
   まま出し、入りきらないぶんはそのまま小さく出す（広げるのは閲覧器の
   指の動きに任せる。横ずらしの許容はこのため）。PC では毎回消す ──
   決めた大きさが残ると、枠いっぱいに広がらなくなる */
function mobPaperFit(box, ratio){
  /* **いったん消してから測る。** 決めた大きさが残ったまま測ると、
     前に決めた値を場所と読み違えて、どんどん外れていく。 */
  box.style.removeProperty("width");
  box.style.removeProperty("height");
  if(!document.body.classList.contains("mob")) return;
  /* **場所を測るために、いったん枠いっぱいに広げる。**
     真ん中寄せ（place-self:center）のままだと、中身の広がりを
     場所と読み違える。100% は紙の置き場（行と列のひと枠） */
  box.style.width = "100%";
...
function fitGrade(){
  ...
  if(!box || !sh) return;
  /* 携帯では A4 よこの縦横のまま出す（枠のかたちに潰さない） */
  mobPaperFit(box, GV_PAGE.w / GV_PAGE.h);
...
function fitMonth(cell){
  ...
  if(cell){
    ...
    box.style.gridTemplateRows    = "repeat(2," + cell.h + ")";
    ...
    box.style.removeProperty("width");
    /* 携帯では B4 の縦横のまま出す（枠のかたちに潰さない） */
    mobPaperFit(box, M_PAGE.w / M_PAGE.h);
...
function fitCal(cell){
  ...
  if(cell){
    box.style.gridTemplateColumns = "repeat(2," + cell.w + ")";
    box.style.gridTemplateRows    = cell.h;
    box.style.width = "calc(" + cell.w + "*2 + 6mm)";
  }else{
    box.style.removeProperty("grid-template-columns");
    box.style.removeProperty("grid-template-rows");
    box.style.removeProperty("width");
    /* 携帯では A4 よこの縦横のまま出す（枠のかたちに潰さない） */
    mobPaperFit(box, CAL_PAGE.w / CAL_PAGE.h);
    if(box.clientWidth < 2 || box.clientHeight < 2) return;
  }
